...
$virtKVM = new virtKVM();
class virtKVM extends Controller
{
    public function createVM($userid, $paket){
        ...
        curl_setopt($ch, CURLOPT_POSTFIELDS, "addvs=1&virt=proxk&rootpass=". $rootkey ."&hostname=". $hostname ."&space=". $storage ."&ram=". $ram ."&swap=0&bandwidth=0&cpu=1024&cores=". $cores ."&ips=1&vnc=0&osid=881&uid=82");
        ...
        if($error == NULL){
            $SQL = self::db()->prepare("INSERT INTO `kvmserver`(`plan_id`, `user_id`, `virt_id`, `serv_ip`, `root_pw`, `hostname`, `state`, `expire_at`, `price`) VALUES (?,?,?,?,?,?,?,?,?)");
    ...
    public function stopKVM($node_id, $virtid)
    {
    ...
    public function startKVM($node_id, $virtid)
    {
    ...
    public function rebootKVM($node_id, $virtid)
    {
    ...
    public function changeRootKey($node_id, $virtid, $rootpass)
    {
    ...
    public function changeHOSTname($node_id, $virtid, $hostname)
    {
    ...
    public function reinstallKVM($node_id, $virtid, $osid)
    {
    ...
    public function deleteKVM($virtid)
    {
